<x-pulse::card :cols="$cols" :rows="$rows" :class="$class">
    <x-pulse::card-header
            name="Slow Mongo Queries"
            title="Time: {{ number_format($time) }}ms; Run at: {{ $runAt }};"
            details="{{ $config['threshold'] }} microseconds threshold, past {{ $this->periodForHumans() }}"
    >
        <x-slot:icon>
        ...
        @else
            <x-pulse::table>
                ...
                @foreach ($slowQueries->take(100) as $query)
                    <tr wire:key="{{ md5($query->command.$query->location) }}-spacer" class="h-2 first:h-0"></tr>
                    <tr wire:key="{{ md5($query->command.$query->location) }}-row">
                        <x-pulse::td class="!p-0 truncate max-w-[1px]">
                            <div class="relative">
                                <div class="bg-gray-700 dark:bg-gray-800 py-4 rounded-md text-gray-100 block text-xs whitespace-nowrap overflow-x-auto [scrollbar-color:theme(colors.gray.500)_transparent] [scrollbar-width:thin]">
                                    <code class="px-3">{{ $query->command }}</code>
                                    @if ($query->location)
                        ...
                        <x-pulse::td numeric class="text-gray-700 dark:text-gray-300">
                            ...
                            @else
                                <strong>{{ number_format($query->slowest) }}</strong> microseconds
                            @endif
                        </x-pulse::td>
                    </tr>
                @endforeach
